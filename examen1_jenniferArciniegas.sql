...
create table departamento(id int(10) auto_increment primary key, nombre varchar(50) not null);
create table profesor (id int(10) auto_increment primary key, nif varchar(9),
nombre varchar(25), apellido1 varchar(50), apellido2 varchar(50), ciudad varchar(25),  direccion varchar(50), telefono varchar(9), fecha_nacimiento date,
sexo enum("H", "M"), id_departamento int, foreign key(id_departamento) references departamento(id));

create table alumno (id int(10) auto_increment primary key, nif varchar(9), nombre varchar(25), apellido1 varchar(50), apellido2 varchar(50), ciudad varchar(25), 
direccion varchar(50), telefono varchar(9), fecha_nacimiento date, sexo enum("H", "M"));

create table grado(id int(10) auto_increment primary key, nombre varchar(100) not null);

create table asignatura (id int(10) auto_increment primary key, nombre varchar(100), creditos float, tipo enum("básica", "obligatoria", "optativa", "NULL"),
curso tinyint(3), cuatrimestre tinyint(3), id_profesor int, id_grado int, foreign key(id_profesor) references profesor(id), foreign key(id_grado) references grado (id));

create table curso_escolar (id int(10) auto_increment primary key, anyo_inicio year(4), anyo_fin year(4));

create table alumno_se_matricula_asignatura (id_alumno int(10), id_asignatura int(10), id_curso_escolar int(10), foreign key(id_alumno) references  alumno(id), 
foreign key(id_asignatura) references  asignatura(id), foreign key(id_curso_escolar) references curso_escolar(id));
...
order by d.nombre desc, p.apellido1 desc, p.apellido2 desc, p.nombre desc;
...
from asignatura a 
right join profesor p on p.id = a.id_profesor 
WHERE a.id_profesor is null;

/* 5. Devuelve un listado con las asignaturas que no tienen un profesor asignado.*/

SELECT a.nombre as "asignaturas sin profesor asignado"
from asignatura a 
where a.id_profesor is null;
...
SELECT COUNT(*) as "cantidad de alumnos que nacieron en 1999" 
from alumno a 
 ...
 */
SELECT  d.nombre as departamento, COUNT(p.id_departamento) as cantidad
from profesor p 
left join departamento d on d.id = p.id_departamento 
GROUP BY p.id_departamento
ORDER by cantidad desc;

/* 10. Devuelve un listado con todos los departamentos y el número de profesores que hay en cada uno de ellos. Tenga en cuenta que pueden 
 ...
 */
SELECT g.nombre, COUNT(a.id_grado)  as cantidad
from asignatura a 
